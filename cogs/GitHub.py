...
class GitHubFetcher(commands.Cog):
    ...
    async def addRepo(self, ctx, repoOwner: str = None, repoName: str = None):
        ...
        successEmbed.add_field(name='**Status**', value='Successfully added Repo to the list', inline=False)
        successEmbed.add_field(name='**Repository**', value="https://github.com/"+repoOwner+"/"+repoName, inline=False)
        await ctx.send(embed=successEmbed)
        lg.info(f"Added {repoOwner}/{repoName} to the list of repositories")
        return
        ...
        if really == 0:
            ...
            for i in self.targetURL:
                ...
                if urlBits[5] == repoName:
                    ...
                    await ctx.send(embed=successEmbed)
                    lg.info(f"Deleted {repoName} from the list of repositories")
                    return
    ...
    async def listRepos(self, ctx):
        ...
        lg.info("Listing all repositories")
        for i in self.targetURL:
            urlBits = i.split('/')
