...
class GitHubFetcher(commands.Cog):
    ...
    @commands.command(name='addRepo', help="Adds a repo to the list of repos")  # help message
    ...
    async def addRepo(self, ctx, repoOwner: str = None, repoName: str = None):
        ...
        successEmbed.add_field(name='**Repository**', value="https://github.com/" + repoOwner + "/" + repoName,
                               inline=False)
        await ctx.send(embed=successEmbed)
        return

    @commands.command(name='deleteRepo', help="Deletes a repo from the list of repos")  # help message
    @commands.has_role('RepoManager')
    @commands.guild_only()
        ...
        if really == 0:
            ...
            for i in self.targetURL:
                ...
                if urlBits[5] == repoName:
                    ...
                    successEmbed.add_field(name='**Status**', value='Successfully removed repo from list!',
                                           inline=False)
                    await ctx.send(embed=successEmbed)
                    return

    @commands.command(name='listRepos', help="Lists all the stored repos")  # help message
    @commands.has_role('RepoManager')
    @commands.guild_only()
    ...
    @commands.command(name='getCommits', help="Returns the last commits in the repository")  # help message
    @commands.has_role('RepoManager')
    @commands.guild_only()
    async def getCommits(self, ctx, repoNum: int, commitAmount: int):
        ...
        if not commits:
            ...
            errorEmbed.add_field(name='**Details**', value='Malformed URL ! (Repo doesnt exist or is private?)',
                                 inline=False)
            await ctx.send(embed=errorEmbed)
            return
...
def setup(bot):
    lg.info(f"GitHound by mass1ve-err0r - https://www.github.com/mass1ve-err0r/GitHound")
